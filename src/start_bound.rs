...
impl<T, D> From<Bound<T>> for StartBound<T, D>
where
    D: Discreteness,
{
...
impl<T, D> Hash for StartBound<T, D>
where
    T: Hash,
{
...
impl<T, D> Eq for StartBound<T, D>
where
    T: PartialEq<T>,
    D: Discreteness,
{
...
impl<T, D> PartialEq<StartBound<T, D>> for StartBound<T, D>
where
    T: PartialEq<T>,
    D: Discreteness,
{
...
impl<T, D> PartialEq<EndBound<T, D>> for StartBound<T, D>
where
    ...
    D: Discreteness,
{
    fn eq(&self, other: &EndBound<T, D>) -> bool {
        ...
        match (self.bound(), other.bound()) {
            ...
            (Bounded(s), Unbounded) => s.eq(&T::max_value()),
            (Unbounded, Bounded(t)) => T::min_value().eq(t),
            (Unbounded, Unbounded) => false,
        }
    }
}

impl<T, D> PartialOrd<StartBound<T, D>> for StartBound<T, D>
where
    ...
    D: Discreteness,
{
    fn partial_cmp(&self, other: &StartBound<T, D>) -> Option<Ordering> {
        ...
        match (self.bound(), other.bound()) {
            ...
            (Unbounded, Bounded(t)) => T::min_value().partial_cmp(t),
            (Unbounded, Unbounded) => Some(Ordering::Equal),
        }
    }
}

impl<T, D> PartialOrd<EndBound<T, D>> for StartBound<T, D>
where
    T: PartialOrd<T> + Bounded,
    D: Discreteness,
{
    fn partial_cmp(&self, other: &EndBound<T, D>) -> Option<Ordering> {
        ...
        match (self.bound(), other.bound()) {
            ...
            (Unbounded, Bounded(t)) => T::min_value().partial_cmp(t),
            (Unbounded, Unbounded) => Some(Ordering::Less),
        }
    }
}
...
mod tests {
    ...
    mod vs_start_bound {
        ...
        fn unbounded_bounded() {
            ...
            assert_eq!(ordering(isize::MAX), Some(Less));
    ...
    mod vs_end_bound {
        ...
        fn bounded_unbounded() {
            ...
            assert_ne!(Lhs::bounded(2), Rhs::unbounded());
            assert_ne!(Lhs::bounded(3), Rhs::unbounded());
            assert_eq!(Lhs::bounded(isize::MAX), Rhs::unbounded());
        }
        ...
        fn unbounded_bounded() {
            ...
            assert_ne!(Rhs::unbounded(), Lhs::bounded(2));
            assert_ne!(Rhs::unbounded(), Lhs::bounded(3));
            assert_eq!(Rhs::unbounded(), Lhs::bounded(isize::MAX));
        }

        #[test]
        fn unbounded_unbounded() {
            assert_eq!(Lhs::unbounded().partial_cmp(&Rhs::unbounded()), Some(Less));
